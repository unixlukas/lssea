...
if [ -n "$SEA" ]; then
  ...
  for en in $(netstat -ni | awk '{ print $1 }' | egrep -v "^Name|^lo0|\*$"|uniq)
   do
     ...
     ent=ent${en#en}
        # check if it physical port or channel / added by L. Schmid 2022.11.08
        if lscfg -l $ent>/dev/null 2>/dev/null; [ "$?" -eq 0 ]; then
        printf " it's a physical or virtual Adapter"
        SLOT=$( lscfg -l $ent|cut -d"-" -f3|cut -c2-)
        else
        print " looks like an Etherchannel .."
        fi
